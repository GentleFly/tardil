...
proc ::tardil::generate_with_latency {args} {
    ...
    foreach clock_000 ${clocks_000} {
        ...
        set strigns [lappend strigns "# vivado: config_timing_pessimism -common_node off\n"]
        set strigns [lappend strigns "# Extended Useful Skew\n"]
        set strigns [lappend strigns "# File generated by command: [info level 0]\n"]
        set strigns [lappend strigns "\n\n# Extended Useful Skew for clock: ${orig_clock_name}\n"]
...
"]
    ...
    set shifts [list]
    set original_clocks [list]
    set clks [lsearch -regexp -inline -all ${clocks} ".*_${prefix}_(n|p)\[0-9]*"]
    foreach clk ${clks} {
        ::tardil::pattern_to_name_and_shift ${clk} original_clock current_shift
        set shifts [lappend shifts ${current_shift}]
        set original_clocks [lappend original_clocks ${original_clock}]
    }
    set shifts [lsort -dictionary -uniq ${shifts}]
    set original_clocks [join [lsort -dictionary -uniq ${original_clocks}] " "]
    ...
    foreach current_shift ${shifts} {
        ...
        foreach another_shift ${another_shifts} {
            if { ${current_shift} <= ${another_shift} + 359 } {
                ...
                # get name of origin names
                if { ${current_shift} == 0 } {
                    set current_original_clocks " ${original_clocks}"
                } else {
                    set current_original_clocks ""
                }
                if { ${another_shift} == 0 } {
                    set another_original_clocks " ${original_clocks}"
                } else {
                    set another_original_clocks ""
                }
                ...
                set start_name "*_${prefix}_${sign_current_shift}[format %0.4u [expr abs(${current_shift})]]${current_original_clocks}"
                set end_name   "*_${prefix}_${sign_another_shift}[format %0.4u [expr abs(${another_shift})]]${another_original_clocks}"
                set strigns [lappend strigns "
...
set_multicycle_path [expr ${cnt_cycles}-1] -hold -from \[get_clocks {${start_name}}\] -to \[get_clocks {${end_name}}\]"]
                if { ${current_shift} == 0 } {
                set strigns [lappend strigns "
set_multicycle_path ${cnt_cycles} -setup -from \[get_clocks -regexp -filter \"NAME!~.*_${prefix}_.*\"\] -to \[get_clocks {${end_name}}\]
set_multicycle_path [expr ${cnt_cycles}-1] -hold -from \[get_clocks -regexp -filter \"NAME!~.*_${prefix}_.*\"\] -to \[get_clocks {${end_name}}\]"]
                }
                if { ${another_shift} == 0 } {
                set strigns [lappend strigns "
set_multicycle_path ${cnt_cycles} -setup -from \[get_clocks {${start_name}}\] -to \[get_clocks -regexp -filter \"NAME!~.*_${prefix}_.*\"\]
set_multicycle_path [expr ${cnt_cycles}-1] -hold -from \[get_clocks {${start_name}}\] -to \[get_clocks -regexp -filter \"NAME!~.*_${prefix}_.*\"\]"]
                }
            }
        }
    }
